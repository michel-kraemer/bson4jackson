...
/**
 * Deserializes BSON Code objects
 * @author Michel Kraemer
 * @since 2.8.0
 */
public class BsonCodeDeserializer extends JsonDeserializer<Code> {
	@Override
	@SuppressWarnings("deprecation")
	public Code deserialize(JsonParser jp, DeserializationContext ctxt)
			throws IOException {
		if (jp instanceof BsonParser) {
			...
			if (bsonParser.getCurrentToken() != JsonToken.VALUE_EMBEDDED_OBJECT ||
				...
				throw ctxt.mappingException(Code.class);
			}
			return (Code)bsonParser.getEmbeddedObject();
		} else {
			...
			if (scopeNode instanceof ObjectNode) {
				...
				return new CodeWithScope(code, doc);
			} else {
