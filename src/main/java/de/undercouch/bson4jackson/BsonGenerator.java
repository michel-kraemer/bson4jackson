...
public class BsonGenerator extends JsonGeneratorBase {
	...
	public enum Feature {
		/**
		 * <p>
		 * Enables streaming by setting the document's total number of bytes in
		 * the header to 0. This allows the generator to flush the output buffer
		 * from time to time. Otherwise the generator would have to buffer the
		 * whole file to be able to calculate the total number of bytes.
		 * </p>
		 * <p>
		 * <b>ATTENTION:</b> By enabling this feature, the BSON document
		 * generated by this class will not be compatible to the specification!
		 * However, if you know what you are doing and if you know that the
		 * document will be read by a parser that ignores the total number of
		 * bytes anyway (like {@link BsonParser} or
		 * <code>org.bson.BSONDecoder</code> from the MongoDB Java Driver do)
		 * then this feature will be very useful.
		 * </p>
		 */
		ENABLE_STREAMING;
	...
	private static class DocumentInfo {
		...
		/**
		 * The current position in the array or -1 if the document is no array
		 */
		int currentArrayPos;
		 ...
		 *            the position of the document's header in the output buffer
		 * @param array
		 *            true if the document is an array
		 */
		public DocumentInfo(int headerPos, boolean array) {
			this.headerPos = headerPos;
			currentArrayPos = (array ? 0 : -1);
		}
	}
	...
	/**
	 * Since a BSON document's header must include the size of the whole
	 * document in bytes, we have to buffer the whole document first, before we
	 * can write it to the output stream. BSON specifies LITTLE_ENDIAN for all
	 * tokens.
	 */
	protected final DynamicOutputBuffer _buffer = new DynamicOutputBuffer(
			ByteOrder.LITTLE_ENDIAN);

	/**
	 * Saves the position of the type marker for the object currently begin
	 * written
	 */
	 ...
	 *            {@link org.codehaus.jackson.JsonGenerator.Feature}s are
	 *            enabled.
	 * @param bsonFeatures
	...
	protected boolean isArray() {
		return (_documents.isEmpty() ? false
				: _documents.peek().currentArrayPos >= 0);
	}

	/**
	 * Retrieves and then increases the current position in the array currently
	 * being generated
	 * 
	 * @return the position (before it has been increased) or -1 if the current
	...
	/**
	 * Writes the BSON document header to the output buffer at the given
	 * position. Does not increase the buffer's write position.
	 * 
	 * @param pos
	 *            the position where to write the header
	 */
	 ...
	 * 
	 * @param array
	 *            true if the embedded object is an array
	 * @throws IOException
	...
	/**
	 * If the generator is currently processing an array, this method writes the
	 * field name of the current element (which is just the position of the
	 * element in the array)
	 * 
	 * @throws IOException
	 *             if the field name could not be written
	 */
	...
	/**
	 * Tries to flush the output buffer if streaming is enabled. This method is
	 * a no-op if streaming is disabled.
	 * 
	 * @throws IOException
	 *             if flushing failed
	 */
